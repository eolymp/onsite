...
build:
	go build -ldflags "-X main.version=${BUILD_TAG} -X main.commit=${BUILD_COMMIT} ${BUILD_LD_FLAGS}" ${BUILD_ARGS} \
		-o ${BUILD_PREFIX}eolymp-onsite${BUILD_SUFFIX} ./
...
.PHONY: ship
ship: build
	docker build --platform amd64 -t ghcr.io/eolymp/onsite/onsite:${BUILD_TAG} .
	docker push ghcr.io/eolymp/onsite/onsite:${BUILD_TAG}

	rm ${BUILD_PREFIX}eolymp-onsite${BUILD_SUFFIX}